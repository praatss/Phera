...
(() => {
  ...
  function configureLayout() {
    // Use the pinned Music → Memory → Voice story on normal desktop/tablet
    // viewports. The old code could silently disable it when a scene exceeded
    // the available space by only 2px, which made the effect seem broken.
    const enoughSpace = window.innerWidth >= 760 && window.innerHeight >= 640;
  ...
  window.addEventListener('pageshow', configureLayout);
