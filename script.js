...
(() => {
  ...
  function configureLayout() {
    // Small/short screens and reduced motion get an ordinary scrolling layout.
    const enoughSpace = window.innerWidth >= 760
      ? window.innerHeight >= 680
      : window.innerWidth >= 360 && window.innerHeight >= 820;
    pinned = enoughSpace && !pausedMotion && !reduced.matches;
    body.classList.toggle('pinned', pinned);
    if (pinned) {
      const available = $('.scene-stack').clientHeight;
      // Text enlargement must never clip a chapter inside a fixed viewport.
      if (scenes.some((scene) => scene.scrollHeight > available + 2)) {
        pinned = false;
  ...
  function updateMotion() {
    ...
    if (insideStory && wasPinned !== pinned && visibleChapter >= 0) {
      if (pinned) {
        const { start, travel } = geometry();
        window.scrollTo({ top: start + travel * (visibleChapter + .08) / 3, behavior: 'instant' });
      } else scenes[visibleChapter].scrollIntoView({ behavior: 'instant', block: 'start' });
      renderScroll();
    }
  }
